...
function setData(data){
    var str = '<tr><th>TN</th><th>DESTINATION</th><th>LOCATION</th><th>ADDRESS</th><th>LAST UPDATE</th><th class="edit-el" style="display: none;">ACCEPT</th></tr>';
    for(var i = 0; i < data.length; i++){
        str += '<tr><td>'+ data[i].id +'</td>'+
            ...
            '<td class="edit-el" style="display: none;"><a href="/admin/incoming/accept/'+ data[i].id +'" class="asd4" onclick="alink(event, `accept`);return false;">&#10004;</a></td>'+
            '</tr>';
    }
...
function refresh(){
    sendAjax("POST", "/admin/incoming", false, false, false, function(data){
        data = JSON.parse(data);
        setData(data);
    })
}
...
search.onkeyup = function(e){
    ...
    sendAjax("POST", "/admin/incoming/search/"+ search.value, false, false, false, function(data){
        data = JSON.parse(data);
        setData(data);
    })
}